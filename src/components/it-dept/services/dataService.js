...
export class DataService {
  ...
  exportCSV(profiles, feedbacks) {
    const headers = [
      ...
      'Toughest Courses', 'Challenges 100L', 'Volunteer Roles', 'Committee Interests',
      'CR Continue', 'CR Reason', 'ACR Continue', 'ACR Reason',
    ...
    profiles.forEach(p => {
      ...
      rows.push([
        ...
        (p.volunteerRoles || p.committees || []).join('; '),
        (p.committees || []).join('; '),
        p.crRecommendContinue || matched?.crRecommendContinue || '',
        p.crRecommendReason || matched?.crRecommendReason || '',
        p.acrRecommendContinue || matched?.acrRecommendContinue || '',
        p.acrRecommendReason || matched?.acrRecommendReason || '',
    ...
    feedbacks.filter(f => f.isAnonymous || !f.studentId).forEach((anon, idx) => {
      rows.push([
        ...
        anon.crRecommendContinue || '',
        anon.crRecommendReason || '',
        anon.acrRecommendContinue || '',
        anon.acrRecommendReason || '',
        anon.suggestions200L || '',
        anon.createdAt || '',
        'Anonymous',
